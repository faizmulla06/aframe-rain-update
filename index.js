...
AFRAME.registerComponent('rain', {
	...
	initMeshes: function () {
		...
		function createDropGeometry() {
			...
			topMesh.geometry.merge(bottomMesh.geometry)

			topMesh.geometry.lookAt(fallingVector.sub(vector.normalize()));

			geometry.copy(new THREE.BufferGeometry().clone(topMesh.geometry));
			return geometry;
		}
		...
		function initDropGeometry(geometry) {
			...
			geometry.setAttribute('position', new THREE.BufferAttribute(new Float32Array(geometry.vertices), 3));
			...
			geometry.setAttribute('translate',
				new THREE.InstancedBufferAttribute(translateArray, 3, true));

			geometry.setAttribute('vector',
				new THREE.InstancedBufferAttribute(vectorArray, 3, true));

			geometry.setAttribute('timeOffset',
				new THREE.InstancedBufferAttribute(timeOffsetArray, 1, true));
			geometry.setAttribute('opacity2',
				new THREE.InstancedBufferAttribute(opacityArray, 1, true));
		}

		function createSplashGeometry() {
			var geometry = new THREE.InstancedBufferGeometry();
			geometry.copy(new THREE.SphereGeometry(data.dropRadius));
			return geometry;
		}
